...
static int ln8000_check_status(struct ln8000_info *info)
{
	...
	if (!info->pdata->tbat_mon_disable || !info->pdata->tbus_mon_disable) {
		info->tbus_tbat_fault = LN8000_STATUS(
			val[1], LN8000_MASK_NTC_SHUTDOWN_STS); //tbus or tbat
		info->tbus_tbat_alarm = LN8000_STATUS(
			val[1], LN8000_MASK_NTC_ALARM_STS); //tbus or tbat
	}
	...
	if (info->vbat_regulated || info->iin_regulated || info->tdie_fault ||
	    info->tdie_alarm || info->iin_rc || info->wdt_fault ||
	    info->vbat_ov || info->vac_unplug || info->vac_ov ||
	    info->vbus_ov || info->iin_oc) {
...
static int ln8000_init_device(struct ln8000_info *info)
{
	...
	vbat_float = info->pdata->bat_ovp_th * 100 /
		     102; /* ovp thershold = v_float x 1.02 */
	...
	ln8000_update_reg(info, LN8000_REG_REGULATION_CTRL, 0x3 << 2,
	...
	ln8000_enable_tbus_monitor(
		info, !info->pdata->tbus_mon_disable); //+enables ADC ch
	ln8000_enable_tbat_monitor(
		info, !info->pdata->tbat_mon_disable); //+enables ADC ch
...
#if 0 /* we are recommanded Dual-standalone mode */
	...
	if (info->dev_role == LN_ROLE_MASTER && !info->standalone_mode_master) {
		...
		ln8000_write_reg(info, LN8000_REG_LION_CTRL, 0x00);
	} else if (info->dev_role == LN_ROLE_SLAVE && !info->standalone_mode_slave) {
		ln8000_write_reg(info, LN8000_REG_LION_CTRL, 0xAA);
	...
	ln8000_read_reg(info, LN8000_REG_REGULATION_CTRL,
			&info->regulation_ctrl);
...
static int psy_chg_get_ti_alarm_status(struct ln8000_info *info)
{
	...
	ln_info("adc_vin=%d(th=%d), adc_iin=%d(th=%d), adc_vbat=%d(th=%d), v_offset=%d, alarm=0x%x\n",
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TI_REG_STATUS:
		...
		break;
	case POWER_SUPPLY_PROP_TI_RESET_CHECK:
...
static int psy_chg_set_bus_protection_for_qc3(struct ln8000_info *info,
					      int hvdcp3_type)
{
...
static ssize_t bq2597x_show_diff_ti_bus_current(struct device *dev,
						...
						char *buf)
{
	...
	if (info->dev_role == LN_ROLE_MASTER) {
		...
		if (!bq2597x_slave) {
			bq2597x_slave =
				power_supply_get_by_name("bq2597x-slave");
			if (!bq2597x_slave) {
				ln_err("failed get bq2597x-slave \n");
				return 0;
			}
			ln_err("success get bq2597x-slave \n");
		}
		rc = power_supply_get_property(
			bq2597x_slave, POWER_SUPPLY_PROP_TI_BUS_CURRENT, &pval);
		if (rc < 0) {
			ln_err("failed get bq2597x-slave ti_bus_current \n");
			return -EINVAL;
		}
		ti_bus_current_slave = pval.intval;
		/*get bq2597x_master ti_bus_current*/
		rc = ln8000_get_adc_data(info, LN8000_ADC_CH_IIN,
					 &info->iin_uA);
		//rc = bq2597x_get_adc_data(bq, ADC_IBUS, &result);
		if (!rc)
			ti_bus_current_master = info->iin_uA / 1000;
		/* get diff_ti_bus_current = ti_bus_current_master - ti_bus_current_slave */
		if (ti_bus_current_master > ti_bus_current_slave)
			diff_ti_bus_current =
				ti_bus_current_master - ti_bus_current_slave;
		else
			diff_ti_bus_current =
				ti_bus_current_slave - ti_bus_current_master;
	} else if (info->dev_role != LN_ROLE_MASTER) {
...
static DEVICE_ATTR(diff_ti_bus_current, 0660, bq2597x_show_diff_ti_bus_current,
		   NULL);
...
static int ln8000_create_debugfs_entries(struct ln8000_info *info)
{
	...
	info->debug_root =
		debugfs_create_dir(ln8000_dev_name[info->dev_role], NULL);
	if (!info->debug_root) {
...
static void vac_ov_control_work(struct work_struct *work)
{
	struct ln8000_info *info =
		container_of(work, struct ln8000_info, vac_ov_work.work);
	...
	for (i = 0; i < cnt; ++i) {
		...
		if (enable_vac_ov) {
			...
			if (info->vbus_uV > 10000000) {
				...
				if (adc_check_cnt > 2) {
					enable_vac_ov = 0;
					ln8000_enable_vac_ov(info,
							     enable_vac_ov);
					ln_info("vac_ov=disable, vin=%dmV, i=%d, cnt=%d, delay=%d\n",
						info->vbus_uV / 1000, i, cnt,
						delay);
					adc_check_cnt = 0;
				}
			}
	...
	if (info->pdata->irq_gpio) {
		...
		if (info->client->irq < 0) {
			ln_err("fail to get irq from gpio(irq_gpio=%p)\n",
			       pdata->irq_gpio) info->client->irq = 0;
			return -EINVAL;
		}
...
static const struct of_device_id ln8000_dt_match[] = {
	{ .compatible = "lionsemi,ln8000", .data = (void *)LN_ROLE_STANDALONE },
...
static int try_to_find_i2c_regess(struct ln8000_info *info)
{
	...
	for (i = 0; i < 4; i++) {
		...
		info->regmap = devm_regmap_init_i2c(info->client,
						    &ln8000_regmap_config);
		ret = i2c_smbus_read_byte_data(info->client,
					       LN8000_REG_DEVICE_ID);
		if (ret == 0x42) {
	...
	info->regmap =
		devm_regmap_init_i2c(info->client, &ln8000_regmap_config);
...
static int ln8000_parse_dt(struct ln8000_info *info)
{
	...
	if (info->client->irq == 0) {
		...
		ret = of_property_read_u32(np, "ln8000_charger,irq_index",
					   &prop);
		if (ret < 0) {
	...
	ret = of_property_read_u32(np, "ln8000_charger,bat-ovp-threshold",
				   &prop);
	LN8000_PARSE_PROP(ret, pdata, bat_ovp_th, (prop * 1000 /*uV*/),
			  LN8000_BAT_OVP_DEFAULT);
	ret = of_property_read_u32(np, "ln8000_charger,bat-ovp-alarm-threshold",
				   &prop);
	LN8000_PARSE_PROP(ret, pdata, bat_ovp_alarm_th, (prop * 1000 /*uV*/),
			  0);
	ret = of_property_read_u32(np, "ln8000_charger,bus-ovp-threshold",
				   &prop);
	LN8000_PARSE_PROP(ret, pdata, bus_ovp_th, (prop * 1000 /*uV*/),
			  LN8000_BUS_OVP_DEFAULT);
	ret = of_property_read_u32(np, "ln8000_charger,bus-ovp-alarm-threshold",
				   &prop);
	LN8000_PARSE_PROP(ret, pdata, bus_ovp_alarm_th, (prop * 1000 /*uA*/),
			  0);
	ret = of_property_read_u32(np, "ln8000_charger,bus-ocp-threshold",
				   &prop);
	LN8000_PARSE_PROP(ret, pdata, bus_ocp_th, (prop * 1000 /*uA*/),
			  LN8000_BUS_OCP_DEFAULT);
	ret = of_property_read_u32(np, "ln8000_charger,bus-ocp-alarm-threshold",
				   &prop);
	LN8000_PARSE_PROP(ret, pdata, bus_ocp_alarm_th, (prop * 1000 /*uA*/),
			  0);
	ret = of_property_read_u32(np, "ln8000_charger,ntc-alarm-cfg", &prop);
	LN8000_PARSE_PROP(ret, pdata, ntc_alarm_cfg, prop,
			  LN8000_NTC_ALARM_CFG_DEFAULT);

	/* protection/alarm disable (defaults to enable) */
	pdata->vbat_ovp_disable =
		of_property_read_bool(np, "ln8000_charger,vbat-ovp-disable");
	pdata->vbat_reg_disable =
		of_property_read_bool(np, "ln8000_charger,vbat-reg-disable");
	pdata->iin_ocp_disable =
		of_property_read_bool(np, "ln8000_charger,iin-ocp-disable");
	pdata->iin_reg_disable =
		of_property_read_bool(np, "ln8000_charger,iin-reg-disable");
	pdata->tbus_mon_disable =
		of_property_read_bool(np, "ln8000_charger,tbus-mon-disable");
	pdata->tbat_mon_disable =
		of_property_read_bool(np, "ln8000_charger,tbat-mon-disable");
	pdata->tdie_prot_disable =
		of_property_read_bool(np, "ln8000_charger,tdie-prot-disable");
	pdata->tdie_reg_disable =
		of_property_read_bool(np, "ln8000_charger,tdie-reg-disable");
	pdata->revcurr_prot_disable = of_property_read_bool(
		np, "ln8000_charger,revcurr-prot-disable");
	info->standalone_mode_master = of_property_read_bool(
		np, "ln8000_charger,standalone-mode-master");
	info->standalone_mode_slave = of_property_read_bool(
		np, "ln8000_charger,standalone-mode-slave");

	if (of_property_read_string(np, "charger_name", &chg_name) < 0)
		ln_err("dts no charger name\n");
			...
			const struct i2c_device_id *id)
{
	...
	ret = sysfs_create_group(&info->dev->kobj, &bq2597x_attr_group);
	if (ret) {
		ln_err("failed to register sysfs. err: %d\n", ret);
	}
...
static int ln8000_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(&info->dev->kobj, &bq2597x_attr_group);
